...
if storeFront == true {
	...
	//backDrop = instance_create_layer(view_wport[0] / 2,  view_hport[0] / 2, "GUI", oFurnaceUI);
	...
	draw_text(xx - 100, yy + 20, "Farm Supplies");
	...
	if point_in_rectangle(mx, my, xx + 64, yy + 20, xx + 128, yy + 84) {
		...
		draw_text_ext(xx - 100, yy + 47, "Carrot Seeds", 18, 140);
		draw_sprite_ext(sCoin, 0, xx - 108, yy + 203, _scale, _scale, 0, c_white, 1);
		draw_text(xx - 80, yy + 204, "-10");
	}
    ...
    for (var i = 0; i < _eachItem; i++) {
		...
		if variable_struct_exists(global.Items, _itemNames[i]) {
			...
			if value[$ "amount"] >= 1 {
				...
				if value[$ "type"] != "Farming" {
					draw_set_alpha(0.5);
					draw_rectangle(item_x, item_y, item_x + 64, item_y + 64, 0);
					draw_set_alpha(1);
				}
				
				if point_in_rectangle(mx, my, item_x, item_y, item_x + 64, item_y + 64) && value[$ "type"] == "Farming"  {
					draw_set_color(c_white);
